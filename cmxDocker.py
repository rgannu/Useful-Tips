...
class CmxApi:
            ...
            for hostname in self.host_list:
                if hostname not in [x.hostname for x in host_instance.read_hosts().items]:
                    ...
                    api_response = cm_instance.host_install_command(
                        body=cm_client.ApiHostInstallArguments(host_names=self.host_list, user_name=self.username,
    ...
    def setup_hdfs(self):
        ...
        try:
            ...
            # services_instance.first_run(self.cluster_name, service_name)
    ...
    def setup_hive(self):
        ...
        try:
            ...
            service_config = [{'name': 'hive_metastore_database_host', 'value': 'localhost'},
            ...
            for rcg in api_response.items:
                if rcg.role_type == 'OOZIE_SERVER':
                    service_config = [{'name': 'oozie_database_user', 'value': 'oozie'},
                                      {'name': 'oozie_database_host', 'value': '%s:%s' % ('localhost', '7432')},
    ...
    def cluster_restart(self):
        # create an instance of the API class
        api_instance = cm_client.ClustersResourceApi(self.api_client)
        print "Cluster Restart command..."
        try:
            # Restart services in a cluster.
            api_response = api_instance.restart_command(self.cluster_name,
                                                        body=cm_client.ApiRestartClusterArgs(
    ...
    def deploy_client_config(self):
        # create an instance of the API class
        api_instance = cm_client.ClustersResourceApi(self.api_client)
        print "Deploy Cluster client-config..."
        try:
            # Restart services in a cluster.
            api_response = api_instance.deploy_client_config(self.cluster_name)
            self.wait(api_response)
        except ApiException as e:
            print("Exception : %s\n" % e)
